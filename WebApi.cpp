...
static map<string, HttpApi> usr_map_api;
...
void usr_api_regist(const string &api_path, const function<void(API_ARGS_MAP)> &func) {
    usr_map_api.emplace(api_path, toApi(func));
}

void usr_api_regist(const string &api_path, const function<void(API_ARGS_MAP_ASYNC)> &func) {
    usr_map_api.emplace(api_path, toApi(func));
}

void usr_api_regist(const string &api_path, const function<void(API_ARGS_JSON)> &func) {
    usr_map_api.emplace(api_path, toApi(func));
}

void usr_api_regist(const string &api_path, const function<void(API_ARGS_JSON_ASYNC)> &func) {
    usr_map_api.emplace(api_path, toApi(func));
}

void usr_api_regist(const string &api_path, const function<void(API_ARGS_STRING)> &func) {
    usr_map_api.emplace(api_path, toApi(func));
}

void usr_api_regist(const string &api_path, const function<void(API_ARGS_STRING_ASYNC)> &func) {
    usr_map_api.emplace(api_path, toApi(func));
}
...
static inline void addHttpListener() {
    ...
    NoticeCenter::Instance().addListener(
        &web_api_tag, Broadcast::kBroadcastHttpRequest, [](BroadcastHttpRequestArgs) {
        ...
        try {
            ...
            responseApi(API::SqlFailed, StrPrinter << u8"操作数据库失败，请检查相关参数" , invoker);
        }
#endif// ENABLE_MYSQL
        catch (std::exception &ex) {
...
Value getusrInfo(std::map<string,string> &it) {
    Value tmp;
    for (auto is : it) {
        tmp[is.first] = is.second;
    }
    return tmp;
}
...
void installWebApi() {
    ...
    static auto usr_get_api_list = [](API_ARGS_MAP) {
        CHECK_SECRET();
        for (auto &pr : usr_map_api) {
            val["data"].append(pr.first);
        }
    };
    ...
    //测试用url,获取所有信息
    usr_api_regist("/index/api/usr/getAllinfo", [](API_ARGS_MAP) {
        CHECK_SECRET();
        //ce
         vector<map<string, string>> ret;
         const char *s = "select * from video_save";
         SqlWriter a(s);
         a << ret;
         val["code"] = API::Success;
         ...
         for (auto it:ret)
         {
             val["data"].append(getusrInfo(it));
         }
         
    });
    //获取token
     usr_api_regist("/index/api/usr/getToken", [](API_ARGS_MAP) {
        CHECK_ARGS("pushKey", "usrname");
        // ce
        
        val["code"] = API::Success;
        val["data"].append(usrinfo::UsrToken::Instance().addToken(allArgs["usrname"], allArgs["pushKey"]).getTokenInfo());
        
    });

    //根据用户对应的station_name来获取对应的信息
    usr_api_regist("/index/api/usr/getinfo_station", [](API_ARGS_MAP) {
         CHECK_STATION();
        //ce
        vector<map<string, string>> ret;
        const char *s = "select * from video_save where station_name='?'";
        // string station_id = usrinfo::UsrToken::Instance().getstation(allArgs["station"]);
        SqlWriter a(s);
        a << allArgs["station"];
        a << ret;
        val["code"] = API::Success;
    ...
    //获取对应的时间相关信息
    usr_api_regist("/index/api/usr/gettime", [](API_ARGS_MAP) {
        CHECK_TOKEN();
        val["code"] = API::Success;
        val["data"].append(usrinfo::UsrToken::Instance().getpri(allArgs["token"]).getTimeInfo());
    });



    usr_api_regist("/index/api/usr/getinfo_stream_id", [](API_ARGS_MAP) {
        CHECK_STATION();
        CHECK_ARGS("streamid");
        // ce
        vector<map<string, string>> ret;
        const char *s = "select * from video_save where video_streamId='?'";
        SqlWriter a(s);
        a << allArgs["streamid"];
        a << ret;
        val["code"] = API::Success;
    ...
    //根据时间段来查询，包含起始时间，终止时间start,end。
    usr_api_regist("/index/api/usr/getinfo_time_step", [](API_ARGS_MAP) {
        CHECK_STATION();
        CHECK_ARGS("start", "end");
        // ce
        vector<map<string, string>> ret;
        const char *s = "SELECT * FROM `video_save` WHERE video_date > '?' AND video_date < '?'";
        // string station_id = usrinfo::UsrToken::Instance().getstation(allArgs["station"]);
        SqlWriter a(s);
        a << allArgs["start"];
        a << allArgs["end"];
    ...
    usr_api_regist("/index/api/usr/test5", [](API_ARGS_MAP) {
        if (UsrToken::Instance().checkToken(allArgs["token"])) {
            throw AuthException("token error");
        }
        // ce
    ...
    //获取服务器api列表
    //测试url http://127.0.0.1/index/api/user/getApiList
    usr_api_regist("/index/api/usr/getApiList", [](API_ARGS_MAP) { usr_get_api_list(API_ARGS_VALUE); });
